...
public final class Stream<R: Request>: Streaming {
    ...
    public func start(_ completion: @escaping (Result<CallResult?>) -> Void) {
        ...
        do {
            ...
            case .unary:
                try call.start(request) { [weak self] response in
...
public extension Streaming where Request: UnaryRequest {
    func data(_ completion: @escaping (Result<Request.OutputType>) -> Void) {
...
public extension Streaming where Request: SendRequest, Message == Request.Message {
    ...
    func send(_ message: Message, completion: @escaping (Result<Void>) -> Void) {
        start { [weak self] result in
            guard let me = self else {
                return
            }
...
public extension Streaming where Request: ReceiveRequest {
    func receive(_ completion: @escaping (Result<Request.OutputType?>) -> Void) {
        start { [weak self] result in
            guard let me = self else {
                return
            }
            ...
            func receiveRecursive() {
                do {
                    try me.call.receiveMessage { callResult in
...
public extension Streaming where Request: CloseRequest {
...
public extension Streaming where Request: CloseAndReciveRequest {
